...
def draw_vec_field(surface: pg.Surface, field: np.ndarray[np.float32], arrow_scale: int = 10, color: tuple[float, float, float] = [0, 0, 0]) -> None:
...
def draw_field(surface: pg.Surface, field: np.ndarray[np.float32], color: tuple[float, float, float] = [0, 0, 1]) -> None:
    arr = util.grid(pg.surfarray.pixels3d(surface), *field.shape)
    arr[...] += np.floor(255.0 * field[:, :, None, None, None] * np.array(color, dtype=np.float32)[None, None, None, None, :]).astype(np.uint8)
    

def draw_shape(surface: pg.Surface, body: pm.Body, shape: pm.Shape, scale: float = 1.0, color: tuple[int, int, int] = [0, 0, 0]) -> None:
    ...
    if isinstance(shape, pm.Circle):
        pg.draw.circle(surface, color, util.to_int(pm_to_pg(shape.offset)), shape.radius * scale)
