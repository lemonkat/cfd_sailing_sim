...
def induce(
    ...
    # tf.Assert(tf.math.is_nan(tf.reduce_sum(result)), ["induction"])
    return result
...
@tf.function(reduce_retracing=True)
def hide_the_evidence(arr: tf.Tensor) -> tf.Tensor:
    nan_idx = tf.where(tf.math.is_nan(arr))
    # mean = tf.reduce_mean(arr, axis=None)
    result = tf.tensor_scatter_nd_update(
        arr, nan_idx, tf.zeros(tf.shape(nan_idx)[0], dtype=arr.dtype)
    )
    # tf.Assert(tf.math.is_nan(tf.reduce_sum(result, axis=None)), "eeeeee")
    return result


@tf.function
def step(vel_i: tf.Tensor, vel_j: tf.Tensor, dens: tf.Tensor, bg_vel_i: tf.Tensor, bg_vel_j: tf.Tensor, bg_dens: tf.Tensor, dt: tf.Tensor) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
    # vel_i_1 = advect(vel_i, vel_i, vel_j, dt)
    # vel_j_1 = advect(vel_j, vel_i, vel_j, dt)

    # pv_i, pv_j = project(dens, dt, 10)
    # vel_i_2 = vel_i_1 + pv_i
    # vel_j_2 = vel_j_1 + pv_j

    # dens_1 = advect(dens, vel_i_2, vel_j_2, dt)

    # return vel_i_2, vel_j_2, dens_1
    ...
    return (
        induce(vel_i, bg_vel_i, 3, 1.0),
        induce(vel_j, bg_vel_j, 3, 1.0),
        induce(dens, bg_dens, 3, 1.0),
    )

    # advect density and momentum - not density and velocity
    # tf.Assert(tf.math.is_nan(tf.reduce_sum(vel_i)), ["vel i start"])
    # tf.Assert(tf.math.is_nan(tf.reduce_sum(vel_j)), ["vel j start"])
    # tf.Assert(tf.math.is_nan(tf.reduce_sum(dens)), ["dens start"])

    # mtm_i, mtm_j = vel_i * dens, vel_j * dens

    # mtm_i_1, mtm_j_1 = advect(mtm_i, vel_i, vel_j, dt), advect(mtm_j, vel_i, vel_j, dt)
    # # tf.Assert(tf.math.is_nan(tf.reduce_sum(mtm_i_1)), ["mtm i end"])
    # # tf.Assert(tf.math.is_nan(tf.reduce_sum(mtm_j_1)), ["mtm j end"])

    # dens = advect(dens, vel_i, vel_j, dt)

    # pv_i, pv_j = project(dens, dt, 10)

    # # tf.Assert(tf.math.is_nan(tf.reduce_sum(pv_i)), ["pv i"])
    # # tf.Assert(tf.math.is_nan(tf.reduce_sum(pv_j)), ["pv j"])

    # vel_i = mtm_i_1 / dens + pv_i
    # vel_j = mtm_j_1 / dens + pv_j

    # one = ctt(1.0, dtype=FLOAT)
    # three = ctt(3, dtype=INT)

    # # tf.Assert(tf.math.is_nan(tf.reduce_sum(vel_i)), ["vel i end"])
    # # tf.Assert(tf.math.is_nan(tf.reduce_sum(vel_j)), ["vel j end"])
    # # tf.Assert(tf.math.is_nan(tf.reduce_sum(dens)), ["dens end"])

    # return (
    #     induce(blur(vel_i), bg_vel_i, three, one),
    #     induce(blur(vel_j), bg_vel_j, three, one),
    #     induce(blur(dens), bg_dens, three, one),
    # )
